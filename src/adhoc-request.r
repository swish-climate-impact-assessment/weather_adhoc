...
for(year_i in 2009:2012)
{
dates <- as.character(seq(as.Date(sprintf('%s-01-01', year_i)), as.Date(sprintf('%s-12-31', year_i)), 1))
#                      print(min(dates));print(max(dates))
...
for(date_j in dates)
{
  ...
  if(date_j == dates[1])
  {
    try(dbSendQuery(ch, "drop table  weather_sla.weather_nswsla06"))
  }
...
dat$date <- as.Date(dat$date)
 str(dat)
head(dat)
write.csv(dat, sprintf("weather_nswsla06_%s.csv", year_i), row.names=FALSE)
}
